...
export class AIService {
  ...
  async scanBill(imageBase64?: string, imageUrl?: string): Promise<ScanBillResult> {
    ...
    const response = await apiService.post<ApiResponse<ScanBillResult>>('/ai/scan-bill', payload);
    return response.data.data;
  }

  async autoCategorize(data: AutoCategorizeRequest): Promise<AutoCategorizeResult> {
    const response = await apiService.post<ApiResponse<AutoCategorizeResult>>('/ai/auto-categorize', data);
    return response.data.data;
  }
